...
>
> 1) В разделе **Батарея** выберите режим **Не ограничивать** (Unrestricted).
>
> 2) Выключите тумблер **Приостановить работу приложения если оно не используется**.
>
> 3) В разделе **Разрешения приложений** выключите тумблер **Приостановить работу в неактивный период**.
>
> 4) Рекомендуется разрешить всё, что находится в разделе **Другие разрешения** (ярлыки рабочего стола, экран блокировки, открывать новые окна в фоновом режиме, отображение всплывающих окон).
>
> 5) Рекомендуется разрешить все уведомления.
>    
> 6) Рекомендуется добавить приложение **HTTP Shortcuts** в исключения VPN (если вы используете VPN) а также домен vercel.app, чтобы запрос календаря не шёл через VPN-сервисы.
