...
3. Перетащите его на экран. В открывшемся окне настроек:
   ...
   * В самом низу в поле **When clicked, run shortcut** (При клике запускать ярлык) нажмите и выберите ярлык **`Calendar click`**.
...
* **Автообновление:** В настройках ярлыка включено периодическое выполнение (Periodic Execution), которое раз в 3 часа в фоновом режиме опрашивает сервер и обновляет дату, не расходуя заряд батареи.

> **Совет:** Чтобы Android не усыплял фоновое обновление виджета, зайдите в *Настройки телефона* -> *Приложения* -> *HTTP Shortcuts* и сделайте следующее:
>
> 1) В разделе **Приложения** добавьте **HTTP Shortcuts** в список **Автозапуск в фоновом режиме**
>
> 2) В разделе **Батарея** выберите режим **Не ограничивать** (Unrestricted).
>
> 3) Выключите тумблер **Приостановить работу приложения если оно не используется**.
>
> 4) В разделе **Разрешения приложений** выключите тумблер **Приостановить работу в неактивный период**.
>
> 5) Добавьте приложение **HTTP Shortcuts** в исключения VPN (если вы используете VPN) а также домен vercel.app, чтобы запрос календаря не шёл через VPN-сервисы.
>
> 6) Рекомендуется разрешить всё, что находится в разделе **Другие разрешения** (ярлыки рабочего стола, экран блокировки, открывать новые окна в фоновом режиме, отображение всплывающих окон).
>
> 7) Рекомендуется разрешить все уведомления.

**Первые пять пунктов являются обязательными для стабильного обновления календаря в фоновом режиме.**
